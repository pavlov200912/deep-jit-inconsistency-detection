...
class EmbeddingStore(nn.Module):
    def __init__(self, nl_threshold, nl_embedding_size, nl_token_counter,
                 ...
                 dropout_rate, num_src_embeddings, src_embedding_size, node_embedding_size,
        ...
        self.node_synthesis_layer = nn.Linear(code_embedding_size+src_embedding_size, node_embedding_size, bias=False)
    ...
    def get_node_embeddings(self, lookup_ids, src_ids):
        lookup_embeddings = self.get_code_embeddings(lookup_ids)
        src_embeddings = self.get_src_embeddings(src_ids)

        embeddings = torch.cat([lookup_embeddings, src_embeddings], dim=-1)
