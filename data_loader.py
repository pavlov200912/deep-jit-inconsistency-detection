...
def get_data_splits(comment_type_str=None, ignore_ast=False):
    ...
    dataset, high_level_details = load_processed_data(comment_type_str,
                                                      ignore_ast)
...
def load_cleaned_test_set(comment_type_str=None):
    ...
    for comment_type in comment_types:
        resources_path = os.path.join(DATA_PATH, 'resources', comment_type,
                                      'clean_test_ids.json')
...
def load_processed_data(comment_type_str, ignore_ast):
    ...
    for comment_type in comment_types:
        ...
        category_high_level_details_path = os.path.join(DATA_PATH, 'resources',
                                                        comment_type,
                                                        'high_level_details.json')
        ...
        if not ignore_ast:
            ast_path = os.path.join(DATA_PATH, 'resources', comment_type,
                                    'ast_objs.json')
...
def load_raw_data_from_path(path):
    ...
    for partition in PARTITIONS:
        dataset[partition] = []
        dataset[partition].extend(read_diff_examples_from_file(
            os.path.join(path, '{}.json'.format(partition))))
